...
export const getUserContinent = (location) => {
  ...
  const sorted = Object.keys(COUNTRY_CONTINENT).sort((a, b) => b.length - a.length);
...
const shapeResult = (hospital, serviceScore, serviceReason, locationLabel) => {
  ...
  return {
    ...
    matchScore: Math.min(
      serviceScore + (hospital.verified ? 10 : 0) + (hospital.phoneNumber ? 5 : 0),
      100,
    ),
    matchReason: reasons.length ? reasons.join(' · ') : 'General healthcare facility',
  };
};
...
export const matchHospitals = async (profile, hospitals, topN = 5, symptomMap = null) => {
  ...
  if (!symptomMap) {
    ...
    try {
      const semanticResults = await semanticMatch(queryText, Math.max(topN * 3, 10));
  ...
  for (const hospital of hospitals) {
    const { score: svcScore, reason: svcReason } = scoreServices(hospital, serviceKeywords);
    ...
    if (locationTerms.length && fieldMatches(city, locationTerms)) {
      tier1.push(shapeResult(hospital, totalServiceScore, svcReason, `In ${city}`));
      continue;
    }

    // Tier 2 — country match
    if (locationTerms.length && fieldMatches(country, locationTerms)) {
      tier2.push(shapeResult(hospital, totalServiceScore, svcReason, `In ${country}`));
      continue;
    }

    // Tier 3 — continent match
    if (userContinent && hospital.continent && hospital.continent === userContinent) {
      tier3.push(
        shapeResult(hospital, totalServiceScore, svcReason, `In ${country} (${userContinent})`),
      );
    }
  }
